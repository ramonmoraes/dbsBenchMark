...
class SchemaCreator:
    ...
    def get_identifier(self, model):
        ...
        return SCHEMA_TEMPLATE.format(label=identifier_label, type=self.get_type(True))

    def create_schemas(self):
        schemas = []
        for model in self.models:
            schemas.append(self.get_identifier(model))
            for k, v in model.to_primitive().items():
                index_type = self.get_type(v)
                if index_type:
                    schema = SCHEMA_TEMPLATE.format(label=k, type=index_type)
